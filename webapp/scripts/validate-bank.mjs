...
const COURSES = [
  ...
  },
  {
    ...
    teachesLedger: true,
    // The theory asks gathered at the front of the solutions book, written by
    // scripts/author/ift222_theory_asks.py. Checked verbatim against the papers,
    // and checked for completeness against the asks this gate finds itself.
    theoryAsks: 'theory-asks.json',
  },
  {
    code: 'PHY121',
    dir: 'phy121',
...
/**
 * The theory asks: every question on the papers that is answered in words
 * rather than in arithmetic, gathered into the front of the solutions book.
 *
 * Two jobs, and the second is the one that matters.
 *
 * 1. VERBATIM. Every quote must appear, character for character, inside an As
 *    printed block of the paper and the part it claims. The file is generated by
 *    slicing the printed text, so this is a second, independent reading of it:
 *    if anyone ever hand-edits a quote into something tidier, the build stops.
 *    A quote is evidence, so it alone is exempt from the no-dash house rule.
 *
 * 2. COVERAGE. This gate goes looking for theory asks ITSELF. It splits every
 *    printed part into its numbered sub-asks, keeps the ones whose verb asks for
 *    words (define, state, list, explain, describe, compare and the rest), and
 *    demands that each one is quoted somewhere in the file. So the section
 *    cannot quietly miss an ask, and a new paper added later fails the build
 *    until its recall marks are answered too.
 */
const THEORY_VERB =
  /\b(what is|what do you understand|what happens|state|list|mention|differentiate|compare|describe|discuss|explain|suggest|define|outline|enumerate)\b/i;

/** A printed ask without its part label, so "c. Explain..." and "Explain..." match. */
function unlabel(s) {
  return String(s)
    .replace(/^[a-z]\.\s*/i, '')
    .replace(/^\(?[ivx]+[.)]\s*/i, '')
...
function subAsks(text) {
  // Parts are printed as "a. i) ... 2mks ii) ... 3mks", and some number their
  // sub-asks "i." or "1." instead. Splitting before each label leaves one piece
  // per ask, with the question's stem as the first piece.
  return text
    .split(/(?=\b(?:i|ii|iii|iv|v|vi|vii)[.)]\s)/)
    .map((s) => s.trim())
    .filter(Boolean);
}

function checkTheoryAsks(cfg, dir, lessonsFile) {
  const path = join(dir, cfg.theoryAsks);
  check(existsSync(path), `${cfg.code}: no theory asks at ${path}`);
  if (!existsSync(path)) return;
  const book = JSON.parse(readFileSync(path, 'utf8'));
  const lessons = JSON.parse(readFileSync(join(dir, lessonsFile), 'utf8'));

  const flat = (s) =>
    String(s ?? '')
      .replace(/<(figure|svg)[\s\S]*?<\/\1>/g, ' ')
      .replace(/<[^>]+>/g, ' ')
      .replace(/&nbsp;/g, ' ')
      .replace(/&#8722;/g, '−')
      .replace(/&#215;/g, '×')
  ...
  for (const l of lessons)
    for (const b of l.blocks ?? [])
      if (b.kind === 'asprinted') printed.push({ tag: b.tag ?? '', text: flat(b.printed) });
  check(printed.length > 0, `${cfg.code}: no printed questions to check the theory asks against`);

  check((book.lead ?? '').length > 40, `${cfg.code} theory asks: lead missing or too thin`);
  check((book.groups ?? []).length > 0, `${cfg.code} theory asks: no groups`);
  ...
  for (const g of book.groups ?? []) {
    const gw = `${cfg.code} theory asks [${g.kind ?? '(no kind)'}]`;
    check(Boolean(g.kind), `${gw}: no kind, so the group has no heading`);
    check((g.when ?? '').length > 40, `${gw}: no "when" line saying what this kind of ask wants`);
    check((g.asks ?? []).length > 0, `${gw}: no asks`);

    for (const a of g.asks ?? []) {
      asks += 1;
      const w = `${gw} ask [${a.id ?? '(no id)'}]`;
      check(/^[a-z0-9-]+$/.test(a.id ?? ''), `${w}: id must be url-safe lower case`);
      check(!ids.has(a.id), `${w}: duplicate id`);
      ids.add(a.id);
      check(Boolean(a.topic), `${w}: no topic, so the answer carries no chip`);
      check(flat(a.answer).length > 60, `${w}: answer missing or too thin to earn its marks`);
      check((a.papers ?? []).length > 0, `${w}: names no paper, so it is not a past question`);

      // Plain text fields render as text: an entity in one reaches the reader
      // as literal characters, the mistake that printed "PART NINE &MIDDOT;".
      for (const k of ['id', 'topic', 'context']) {
        const bad = typeof a[k] === 'string' && a[k].match(/&[a-zA-Z]+;|&#\d+;/);
        if (bad) check(false, `${w}: ${k} contains the entity ${bad[0]}, which renders literally`);
      }
      check(
        !/\^-?[A-Za-z0-9(]/.test(a.answer ?? ''),
        `${w}: the answer writes an exponent as "^", which renders literally. Use <sup>.`
      );
      // House style, with the examiner's own words exempt: a quote is evidence
      // and is printed exactly, dashes, typos and all.
      for (const [k, v] of Object.entries(a))
        if (typeof v === 'string' && k !== 'quote' && /[—–]/.test(v))
          problems.push(`${w}: em or en dash in ${k}, "${v.slice(0, 50)}..."`);

      for (const s of [...(a.papers ?? [])]) {
        const at = `${w} ${s.paper ?? '?'} ${s.part ?? '?'}`;
        check(/^\d{4}\/\d{4}$/.test(s.paper ?? ''), `${at}: paper must read like 2024/2025`);
        check(/^Q\d+\([a-z]\)(\([ivx]+\))?$/.test(s.part ?? ''), `${at}: part must read like Q3(a)(ii)`);
        check(
          (Number.isFinite(s.marks) && s.marks > 0) || (s.marks === null && Boolean(s.note)),
          `${at}: marks must be the number printed on the paper, or null with a note saying why`
        );
        check((s.quote ?? '').length > 15, `${at}: quote missing or too short to be the ask`);

        // 1. VERBATIM, against the paper and the part it claims.
        const [, qn, qa] = (s.part ?? '').match(/^Q(\d+)\(([a-z])\)/) ?? [];
        const short = `${s.paper.slice(2, 4)}/${s.paper.slice(7)}`; // 2024/2025 -> 24/25
        const hits = printed.filter(
          (p) =>
            (p.tag.startsWith(s.paper) || p.tag.startsWith(short)) &&
            (p.tag.includes(`Question ${qn}(${qa})`) || p.tag.includes(`Q${qn}(${qa})`))
        );
        check(hits.length > 0, `${at}: no As printed block belongs to that paper and part`);
        const found = hits.some((p) => p.text.includes(flat(s.quote)));
        if (!found) {
          // Two different mistakes, and saying which one saves the next reader
          // the hunt: a quote nobody printed, or the right quote filed under
          // the wrong part of the paper.
          const elsewhere = printed.find((p) => p.text.includes(flat(s.quote)));
          check(
            false,
            elsewhere
              ? `${at}: this quote is printed under ${elsewhere.tag}, not under the part named here`
              : `${at}: this quote is not in the printed question word for word, so it is a paraphrase: "${flat(
                  s.quote
                ).slice(0, 70)}..."`
          );
        }
        if (found) quotes.push(flat(s.quote));
      }
    }
  }

  // 2. COVERAGE: the gate finds the theory asks in the papers for itself.
  const missed = [];
  for (const p of printed) {
    for (const piece of subAsks(p.text)) {
      // The stem of a question is not an ask; only a labelled sub-ask, or a
      // part that is one ask from end to end, can be one.
      if (!THEORY_VERB.test(piece)) continue;
      if (piece.length < 25) continue;
      const bare = unlabel(piece);
      const covered = quotes.some((q) => {
        const qb = unlabel(q);
        return bare.includes(qb) || qb.includes(bare);
      });
      if (!covered) missed.push(`${p.tag}: ${piece.slice(0, 90)}`);
    }
  }
  const unique = [...new Set(missed)];
  if (unique.length) {
    console.log(`    x theory asks: ${unique.length} asks on the papers are not answered here`);
    for (const m of unique.slice(0, 10)) console.log('        ' + m);
    problems.push(
      `${cfg.code}: ${unique.length} theory ask(s) printed on the papers are missing from ${cfg.theoryAsks}`
    );
  } else {
    console.log(
      `    . theory asks: ${asks} asks, ${quotes.length} quotes verbatim against the papers, and every theory ask on the papers is answered`
    );
  }
}
...
for (const cfg of COURSES) {
  ...
  const KNOWN_NON_BANK = new Set([cfg.lessons, cfg.plan, cfg.theoryAsks, INDEX_FILE, 'paper.json']);
  ...
  if (cfg.theoryAsks) checkTheoryAsks(cfg, dir, cfg.lessons);
