...
<head>
<meta charset="utf-8">
<title>TMC221 - Personal Development and Capacity Building - Study Manual</title>
...
def main():
    ...
    # ---- no-escape coverage gate: every inventory question must be solved ----
    # qa_coverage.py checks question_inventory.md; optional until the full book is
    # authored (a checkpoint solves only Module One's questions).
    try:
        import qa_coverage
        if not qa_coverage.report(draft):
            stop('coverage gate failed: a source question is not yet solved.')
    except ImportError:
        print('COVERAGE GATE: skipped (qa_coverage.py not present yet)')

    # ---- teach-before-use audit: every declared notation (abbreviations, RA symbols,
    # ER/EER terms) is introduced before use and never first in a mock. qa_firstuse.py
    ...
    out = os.path.join(HERE, 'TMC221_manual.pdf')
    ...
    # ---- content gate: every drill option is actually painted in the final PDF ----
    # A drill bank whose options are laid out but dropped is self-consistent and wrong,
    # so measure the rendered artifact, not the HTML. See qa_content.py.
    import qa_content
    if not qa_content.report():
        stop('content QA failed: a drill option is missing from the rendered PDF.')
