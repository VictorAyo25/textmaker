...
These encode the workspace's hard rules:
  ...
  3. The reserved colour (gold #9a6a00) means MUST MEMORISE and nothing
     else, so it may not appear as an inline style in content.
  4. (This course carries no code, so the monospace and code-width checks below
     are dormant guards, kept for portability. The shrink they defend against
     cannot arise without a wide <pre> block.)
"""
...
# Institution branding and the lecturer's name are kept OUT of the manual (house
# style: footer is course + author only). The book AUTHORS the exam actually tests
# (Oyedepo, Leaf, Peale, de Bono, Young) are subject matter and are NOT banned.
BANNED_WORDS = [
    # 'Covenant University', not bare 'Covenant': a slide legitimately uses the
    # common noun ("Dr. Oyedepo's own covenant"), which must not trip the gate.
    'Covenant University', 'CCODeL', 'Canaanland', 'Idiroko',
    'Stroud',
    'ALDC', 'African Leadership Development', 'Ezenwoke',
]
...
def run_gates(html):
    ...
    for m in re.finditer(r'style="[^"]*#9a6a00', html, re.I):
        fails.append('reserved colour #9a6a00 used in an inline style; it belongs '
