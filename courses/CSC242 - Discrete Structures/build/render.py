...
FOOTER = (
    ...
    '<span>CSC242 · Discrete Structures · Victor Ayodeji</span>'
    '<span class="pageNumber" style="color:#1e2830; font-weight:bold; font-size:8pt;"></span></div>'
)
...
with sync_playwright() as p:
    ...
    #
    #    Chromium fetches an @font-face only when something on the page actually
    #    needs it, so a declared family that no glyph currently uses reports
    #    false here even though its file is perfectly good. Measured on this
    #    book: with only serif text on the page, Sans and Sans Mono both read
    #    false, and after the face is requested both read true. So REQUEST every
    #    family we check, then check. Narrowing the guard to the families that
    #    happen to be in use today (the TMC221 fix) would let a genuinely missing
    #    .ttf through the moment the book started using it.
    #
    #    Request them with document.fonts.load(), NOT by appending probe elements
    #    to the page. An earlier version here appended a hidden <span> per family;
    #    `position:absolute` with no offsets leaves an element at its static
    #    position, which on the cover is directly below a box the full height of
    #    the sheet, so the document grew past one page and the cover rendered as
    #    two pages, the second blank. That silently shifted every body page by two
    #    in the merge and left every Contents link pointing one page early. The
    #    font check must not be able to move the layout it is checking.
    #    A face whose .ttf 404s makes fonts.load() REJECT, so each request is
    #    caught individually: a rejection is exactly the "did not load" answer we
    #    are asking for, and letting it escape would abort the render with a raw
    #    NetworkError instead of the explanation below.
    faces = pg.evaluate("""async () => {
        const want = ['DejaVu Sans', 'DejaVu Sans Mono', 'DejaVu Serif'];
        await Promise.all(want.map(f =>
            document.fonts.load('12pt "' + f + '"').catch(() => null)));
