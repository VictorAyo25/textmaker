...
COVER = 'cover.html'
# CHECKPOINT SCOPE: front + Module One. Add modules/tests/mocks as authored, in
# step with contents.py TOC (every part here must supply the headings the TOC names).
BODY_PARTS = ['front.html', 'module1.html', 'module2.html', 'module3.html', 'module4.html', 'reference.html', 'solved_test1.html', 'solved_test2.html', 'mock1.html', 'mock2.html', 'mock3.html', 'mock4.html', 'mock5.html']
...
<head>
<meta charset="utf-8">
<title>ENT221 - Agripreneurship - Study Manual</title>
...
# Force every @font-face to be requested, so render.py's face-loaded guard is
# meaningful even in a section that happens to use no monospace (Module One has no
# code). Without a glyph set in a face, Chromium never fetches it, document.fonts
# reports "loaded" for the faces it did fetch, and the guard then trips on the
# untouched face. This offscreen strip keeps all three families honest and still
# lets a genuine 404 fail the check.
FONT_PRELOAD = (
    '<div aria-hidden="true" style="position:absolute; left:-9999px; top:0; '
    'font-size:6pt;">'
    '<span style="font-family:\'DejaVu Serif\'">.</span>'
    '<span style="font-family:\'DejaVu Sans\'">.</span>'
    '<span style="font-family:\'DejaVu Sans Mono\'">.</span></div>\n')
...
def build_body(contents_html):
    ...
    return HEAD + FONT_PRELOAD + contents_html + marked + TAIL
...
def main():
    ...
    # FONT_PRELOAD forces all three families to be fetched even though the cover
    # uses no monospace, so render.py's face-loaded guard does not trip on the cover.
    cover_html = COVER_HEAD + FONT_PRELOAD + read(COVER) + TAIL
    draft = build_body(build_contents())
    if not run_gates(cover_html + draft):
        stop('house-style gate failed.')

    # ---- no-escape coverage gate (live, hard): both priority papers (25/26 and
    # 24/25) must have every question One to Six cited by a worked-answer chip, and
    # the book must stay diagram-rich above FIG_MIN. Control-tested both ways. It was
    # deferred (qa_coverage.py.pending) during authoring; it is now a hard gate. ----
    # NOTE (ENT221): qa_coverage/qa_firstuse/qa_verbatim are inherited from INS224 and
    # still need adaptation to this course (section-provenance map, ENT221 NOTATIONS, and
    # the MCB/CBT transcript format). Until then they are skippable so the render pipeline
    # (house-style + Contents + layout + font-size) can be validated on a walking skeleton.
    # They become hard gates again in todo #8, each control-tested. Do NOT ship with them off.
    if '--skip-coverage' not in sys.argv:
        try:
            import qa_coverage
            if not qa_coverage.report(draft):
                stop('coverage gate failed: a priority-paper question is not solved, or a '
                     'diagram type has gone text-only.')
        except ImportError:
            print('COVERAGE GATE: skipped (qa_coverage.py not present)')
    else:
        print('COVERAGE GATE: SKIPPED via --skip-coverage (pending ENT221 adaptation)')

    # ---- teach-before-use audit: every declared notation (ERD/UML/DFD abbreviations
    # and symbols) is introduced before use and never first in a mock. qa_firstuse.py
    # carries the per-course NOTATIONS list; a new course rewrites it for its notation. ----
    import qa_firstuse
    ...
    import qa_verbatim
    if not qa_verbatim.report() and '--skip-verbatim' not in sys.argv:
    ...
    out = os.path.join(HERE, 'ENT221_checkpoint.pdf')
    ...
    # ---- rendered-content gate: every declared drill option is painted in the PDF
    # (internal consistency is not correctness: an option laid out but not painted still
    # yields a self-consistent PDF). Runs on the merged PDF, so it goes here. ----
    import qa_content
    if not qa_content.report():
        stop('content QA failed: a declared drill option is not painted in the rendered PDF.')
