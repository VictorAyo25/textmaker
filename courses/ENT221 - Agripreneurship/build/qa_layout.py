...
def check(pdf_path):
    ...
    for i in range(doc.page_count):
        ...
        if i in forced:
            continue                    # this page itself begins a forced section: a module
                                        # or mock divider owns its page (page-break-after), and
                                        # an Answers page just started fresh. Short is by design,
                                        # and nothing may share it, so there is nothing to fix.
        if i + 1 in forced:
...
if __name__ == '__main__':
    src = sys.argv[1] if len(sys.argv) > 1 else 'DTS224_checkpoint.pdf'
