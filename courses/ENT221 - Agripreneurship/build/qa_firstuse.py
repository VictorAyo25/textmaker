...
NOTATIONS = {
    # abbreviations the manual leans on: the intro cue is the spelled-out expansion,
    # written once as "Words (ABBR)". Each must be introduced before a reader meets it,
    # and never first inside a mock.
    # ---- Module Two (fish farming) ----
    'DO':    r'[Dd]issolved [Oo]xygen \(DO\)',
    'TBC':   r'Total Bacterial Count \(TBC\)',
    'USP':   r'Unique Selling Point \(USP\)',
    # ---- Module Three (oil palm) ----
    'FFB':   r'fresh fruit bunches \(FFB\)',
    'CPO':   r'crude palm oil \(CPO\)',
    'PKC':   r'[Pp]alm kernel cake \(PKC\)',
    'PPF':   r'[Pp]alm pressed fibre \(PPF\)',
    'EFB':   r'[Ee]mpty fruit bunches \(EFB\)',
    'NPKMg': r'NPKMg \(12:12:17:2\)',
    'POME':  r'palm oil mill effluent',
    # ---- Module Four (agribusiness) ----
    'POCD':  r'POCD[^A-Za-z]{0,20}framework',
}
