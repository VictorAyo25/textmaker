"""House-style gates for the ENT221 manual. A violation stops the build.
...
These encode the workspace's hard rules:
  ...
  3. The reserved colour (harvest gold #ca8a04) means MUST MEMORISE and nothing
...
ASPRINTED_RE = re.compile(r'(<div class="asprinted"[^>]*>)(.*?)(</div>)', re.S)


def _mask_quotes(html):
    """Blank the text inside every AS PRINTED block to spaces (length preserved so
    error offsets still line up). A quoted exam question may carry the paper's own
    dashes, which our prose may not; masking exempts the quote without disabling the
    check on our prose. Branding stays global. Methodology 4c. Returns (masked, count)."""
    n = [0]
    def sub(m):
        n[0] += 1
        return m.group(1) + re.sub(r'\S', ' ', m.group(2)) + m.group(3)
    return ASPRINTED_RE.sub(sub, html), n[0]


def run_gates(html):
    fails = []
    text = _strip_tags(html)
    dash_html, exempt = _mask_quotes(html)   # dashes are checked on our prose, not on quotes

    # ---- 1. dashes (AS PRINTED quotes masked out) ----
    for ch, name in BANNED_CHARS.items():
        for m in re.finditer(re.escape(ch), dash_html):
            ctx = dash_html[max(0, m.start() - 45):m.start() + 45].replace('\n', ' ')
            fails.append(f'{name} (U+{ord(ch):04X}) found: ...{ctx}...')
    for ent in BANNED_ENTITIES:
        if ent in dash_html:
            i = dash_html.index(ent)
            ctx = dash_html[max(0, i - 45):i + 45].replace('\n', ' ')
    ...
    for m in re.finditer(r'style="[^"]*#ca8a04', html, re.I):
        fails.append('reserved colour #ca8a04 used in an inline style; it belongs '
    ...
    # ---- report ----
    checks = [
        f'no em/en dashes in our prose ({exempt} AS PRINTED quote regions masked)',
