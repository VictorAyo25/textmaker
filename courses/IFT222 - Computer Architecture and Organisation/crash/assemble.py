"""Assemble the IFT222 manual from content/*.html, then render to PDF.
...
COVER = 'cover.html'
# BODY_PARTS grows as skill blocks are authored; it must stay in sync with
# contents.TOC (every heading needs a Contents row and vice versa, or the build
# stops). This is the CRASH edition: lean, organised by exam skill, not syllabus.
BODY_PARTS = ['front.html',
              # Part A: foundations and data (the bedrock comes FIRST, so nothing
              # is used before it is defined: the big picture, then the number work)
              'skill_bigpicture.html',
              'skill_numbers.html',
              'skill_signed.html',
              'skill_ieee.html',
              'skill_image.html',
...
<head>
<meta charset="utf-8">
<title>IFT222 - Exam Crash Course - Zero to a Perfect Score</title>
...
def main():
    ...
    # first-use audit (notation/term before use) is re-added for IFT222 once the
    # teaching sections are in place; the CSC241 version is Python-API specific.
    if os.path.exists(os.path.join(HERE, 'qa_firstuse_ift.py')):
        import qa_firstuse_ift
        if not qa_firstuse_ift.report(draft):
    ...
    out = os.path.join(HERE, 'IFT222_checkpoint.pdf')
