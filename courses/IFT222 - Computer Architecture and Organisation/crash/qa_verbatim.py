...
# The crash edition sits the 2025/2026 paper in full as its final mock, so the
# reverse check binds on it: every question of that paper must be quoted, whole.
...
# The crash edition quotes only the 2025/2026 paper, which has no text-layer PDF
# (it was photographed), so the glyph cross-check has nothing to run against.
_SRC_PDFS = {}
...
def _detable(frag):
    ...
    Re-express it exactly as the transcript stores such a table: cells joined by
    ' | ', one row per line. So a real HTML table in the book still checks
    character for character against the pipe rows in the transcript."""
    def one(m):
        rows = []
