...
4. **Write `qa_verbatim.py`** and wire it into `assemble.py` as a hard gate. It needs **three** directions, not two:
   ...
   The third direction is the one you will not think of, and both other directions are structurally blind to it, because both work from AS PRINTED boxes and these sites have none. COS221 published with two misquotes of exactly this kind, one dropping "(from i onward)" from inside quotation marks and one lowercasing the paper's own quoted term, then republished. IFT222 hit the same gap from the other side and added a discovery check: a worked example whose chip cites a paper must have a quote above it. Take whichever fits your book; do not ship with only two.
5. **Control-test the gate, each direction separately** (non-negotiable 9): change one word in a quoted question; drop one part from the book; change one word inside a prose citation. Confirm each fails on its own. Note that most questions end up quoted **twice** (once where taught, once in the solved paper), so deleting one copy does not test coverage: pick a question quoted exactly once to fire the dropped-part check.
